...
import edu.platform.modules.quiz.service.QuizAttemptService;
import edu.platform.modules.quiz.service.QuizService;
...
public class QuizController {
    ...
    public ResponseEntity<QuizResponse> createQuiz(@Valid @RequestBody QuizRequest request) {
        ...
        User currentUser = userAccessFacade.getUserByEmail(email);
    ...
    private void recordQuizView(QuizResponse quiz, HttpServletRequest request) {
        ...
        try {
            Long userId = userAccessFacade.getUserByEmail(authentication.getName()).getId();
            visitTrackingFacade.recordQuizView(userId, quiz.getCourse().getId(), quiz.getId(), request);
        } catch (Exception ex) {
